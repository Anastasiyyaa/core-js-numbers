...
const safeRound = (number) =>
  Math.round((number + Number.EPSILON) * 1e12) / 1e12;
...
function getAverage(value1, value2) {
  ...
  if (maxAbs > 1e200) {
    ...
    const avg = (value1 * scale + value2 * scale) / 2 / scale;
...
function isPowerOfTwo(num) {
  ...
  while (degree > 2 && isPower) {
    degree /= 2;

    isPower = Number.isInteger(degree);
  }
